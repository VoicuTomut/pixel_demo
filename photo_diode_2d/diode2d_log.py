...
def construct_poisson_eq(device_name, region):
    ...
    # Parameters are correctly fetched
    q = ds.get_parameter(device=device_name, name="q")
    ...
    # This model correctly represents the physical space charge, ρ
    ds.node_model(
        device=device_name, region=region, name="NetDoping",
        equation=f"{N_D} - {N_A}"
    )
    ds.node_model(
    ...
    ds.edge_model(
        device=device_name, region=region, name="ElectricFlux",
        equation=f"{epsilon} * ElectricField"
    )
    ds.edge_model(
        device=device_name, region=region, name="ElectricFlux:Potential@n0",
        equation=f"{epsilon} * ElectricField:Potential@n0"
    )
    ds.edge_model(
        device=device_name, region=region, name="ElectricFlux:Potential@n1",
        equation=f"{epsilon} * ElectricField:Potential@n1"
    )

    # CORRECTION: Define a separate model for the equation's source term, Z = -ρ
    ds.node_model(
        device=device_name, region=region, name="PoissonSource",
    ...
    ds.equation(
        device=device_name, region=region, name="PoissonEquation",
        variable_name="Potential",
        edge_model="ElectricFlux",
        node_model="PoissonSource",  # Use the term with the correct sign
...
def main():
    ...
    try:
        ...
        set_boltzmann_initial_conditions(device_name)
