...
def set_initial_conditions(device_name, region):
    ...
    # CRITICAL: Clamp minimum values to prevent extreme ratios
    MIN_CARRIER = 1e8  # Minimum carrier concentration (cm^-3)
    MAX_CARRIER = 1e20  # Maximum carrier concentration (cm^-3)
...
def build_recombination_model_simplified(device_name, region):
    """
    SIMPLIFIED recombination with more stable derivatives.
    Uses only SRH initially for better convergence.
    """
    print(f"    Building SIMPLIFIED recombination...")
    ...
    # Use intrinsic level for trap energies (midgap)
    n1 = n_i
    p1 = n_i

    # SRH with protective clamping to avoid division issues
    # U = (np - ni^2) / [tau_p*(n + n1) + tau_n*(p + p1)]

    ds.node_model(
        device=device_name, region=region,
        name="U_SRH",
        equation=f"(Electrons*Holes - {n_i_sq}) / "
                 f"({tau_p}*(Electrons + {n1}) + {tau_n}*(Holes + {p1}))"
    )

    # SIMPLIFIED derivatives using quotient rule carefully
    # Let: num = np - ni^2
    #      den = tau_p*(n + n1) + tau_n*(p + p1)
    # Then: dU/dn = (p*den - num*tau_p) / den^2

    ds.node_model(
        device=device_name, region=region,
        name="U_SRH_denom",
        equation=f"{tau_p}*(Electrons + {n1}) + {tau_n}*(Holes + {p1})"
    )

    ds.node_model(
        device=device_name, region=region,
        name="U_SRH:Electrons",
        equation=f"(Holes*U_SRH_denom - (Electrons*Holes - {n_i_sq})*{tau_p}) / "
                 f"(U_SRH_denom * U_SRH_denom + 1e-30)"  # Add small stabilizer
    )

    ds.node_model(
        device=device_name, region=region,
        name="U_SRH:Holes",
        equation=f"(Electrons*U_SRH_denom - (Electrons*Holes - {n_i_sq})*{tau_n}) / "
                 f"(U_SRH_denom * U_SRH_denom + 1e-30)"  # Add small stabilizer
    )

    ds.node_model(
        device=device_name, region=region,
        name="U_SRH:Potential",
        equation="0"
    )

    # Total recombination (just SRH for now)
    ds.node_model(
        device=device_name, region=region,
        name="Recombination",
        equation="U_SRH"
    ...
    ds.node_model(
        ...
        equation="U_SRH:Electrons"
    )

    ds.node_model(
        device=device_name, region=region,
        name="Recombination:Holes",
        equation="U_SRH:Holes"
    )
...
def diagnose_convergence_failure(device_name):
    ...
    for region in ds.get_region_list(device=device_name):
        ...
        print(f"  Electrons: min={min(n_vals):.2e}, max={max(n_vals):.2e}, mean={sum(n_vals) / len(n_vals):.2e}")
        print(f"  Holes:     min={min(p_vals):.2e}, max={max(p_vals):.2e}, mean={sum(p_vals) / len(p_vals):.2e}")
        print(f"  Potential: min={min(V_vals):.3f}V, max={max(V_vals):.3f}V")

        # Check for problem values
        if min(n_vals) <= 0:
            print(f"  ✗ PROBLEM: Non-positive electrons detected!")
        if min(p_vals) <= 0:
            print(f"  ✗ PROBLEM: Non-positive holes detected!")
        if max(n_vals) / min(n_vals) > 1e15:
            print(f"  ✗ PROBLEM: Extreme electron concentration ratio!")
        if max(p_vals) / min(p_vals) > 1e15:
            print(f"  ✗ PROBLEM: Extreme hole concentration ratio!")
